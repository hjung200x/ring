...
import { profileUpsertSchema } from "@ring/schemas";
...
export const interestProfileRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.delete("/:profileId", { preHandler: app.requireSession }, async (request, reply) => {
    const { profileId } = request.params as { profileId: string };
    await service.delete(await getSharedOwnerId(app), profileId);
